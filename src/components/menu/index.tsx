...
import { IComponentProps, IThemeType } from '../../interfaces';
import { Nav, Button, Div } from '../styledComponents';
import { Link, graphql, useStaticQuery } from 'gatsby';
import MenuTitle from './menuTitle';
import MenuItem from './menuItem';
import MenuLayout from './menuLayout';
import { categories, subCategories } from '../../constants/category';
...
const Menu = (props: IMenuProps) => {
  ...
  return (
    <MenuLayout open={props.open} clickButton={props.clickButton}>
      <MenuTitle
        title={`분류 전체(Total) (${data.totalCount})`}
        path='/'
      ...
      >
        <MenuItem
          path={`?category=web`}
          onClick={props.clickButton}
        >{`웹(Web) (${categoryCount.get('web')})`}</MenuItem>
        <MenuItem
          path={`?category=react`}
          onClick={props.clickButton}
        >{`리액트(React) (${categoryCount.get('react')})`}</MenuItem>
        <MenuItem
